...
public:
    ...
    void update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        if( (time.nsec % 5000000) == 0)   // Coment this line when working on the real robot
        {
            ...
            cont+=0.01;
            double pos = 0.0;
            joints_[0].setCommand(0.1*sin(cont) + 0.1);
...
PLUGINLIB_DECLARE_CLASS(controllers_tutorials, NewTorsoController, controllers_tutorials::NewTorsoController, controller_interface::ControllerBase);
